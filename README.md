...
项目接入 `promote.yml` 后，选择 **从构建生成 Release**，填写原来成功构建的 run ID。只复用本仓库“发布软件”流程中已全部成功、尚未过期的产物，重新校验原始提交、工具版本、平台报告、Intel 结果和每个文件哈希，然后生成草稿，不重新编译。

原来的构建若已经创建同版本 Release，提升流程会拒绝覆盖。缺少 Artifact 或原运行失败时也会停止。草稿仍遵守项目的真人验收要求。
