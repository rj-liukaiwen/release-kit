...
export async function orphanedTag(route, sourceSha) {
  try {
    const ref = await api(route);
    if (ref?.object?.type !== 'commit' || ref.object.sha !== sourceSha) {
      throw new Error(`Version already exists: ${route}. It does not match this source commit and will not be reused.`);
    }
    return true;
  } catch (e) {
